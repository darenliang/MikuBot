    ...
    async exec(message: Message, {local, global, page}: { local: boolean, global: boolean, page: number }) {
        ...
        if (local) {
            ...
            if (sortedScores.length == 0) {
                return await message.channel.send('There are no music score entries yet for this server.');
            }
            ...
            if (sortedScores.length == 0) {
                return await message.channel.send('There are no music score entries yet.');
            }
            ...
            if (selectedScores.length == 0) {
                return await message.channel.send(`Your page number is too large. The last page number is ${Math.ceil(sortedScores.length / 10)}.`);
            }
