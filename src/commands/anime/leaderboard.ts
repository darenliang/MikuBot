...
export default class LeaderboardCommand extends Command {
    ...
    async exec(message: Message, {local, global, page}: { local: boolean, global: boolean, page: number }) {
        ...
        if (page < 1) return await message.channel
            ...
            .catch(err => console.log('ERROR', 'leaderboard', 'Failed to send message: ' + err));
        if (local && !message.guild) return await message.channel
            .send('The local leaderboard is only available in servers.')
            .catch(err => console.log('ERROR', 'leaderboard', 'Failed to send message: ' + err));
        if (local) {
            ...
            if (sortedScores.length == 0) return await message.channel
                .send('There are no music score entries yet for this server.')
                .catch(err => console.log('ERROR', 'leaderboard', 'Failed to send message: ' + err));
            ...
            if (selectedScores.length == 0) return await message.channel
                .send(`Your page number is too large. The last page number is ${Math.ceil(sortedScores.length / 10)}.`)
                .catch(err => console.log('ERROR', 'leaderboard', 'Failed to send message: ' + err));
            ...
            if (sortedScores.length == 0) return await message.channel
                .send('There are no music score entries yet.')
                .catch(err => console.log('ERROR', 'leaderboard', 'Failed to send message: ' + err));
